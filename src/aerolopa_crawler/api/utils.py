...
def optimize_image(image_path: str, quality: Optional[int] = None, max_size: Optional[Tuple[int, int]] = None, default_quality: int = 85) -> bytes:
    """优化图片：压缩和调整尺寸
...
def get_cached_image(cache_dir: str, cache_key: str, cache_timeout: int) -> Optional[bytes]:
    """从缓存获取图片
...
def check_local_seatmap_cache(data_dir: str, iata_code: str, aircraft_model: str, image_formats: List[str]) -> List[Dict[str, Any]]:
    """检查本地座位图缓存
...
def filter_aircraft_images(crawl_result: Dict, aircraft_model: str) -> List[Dict[str, Any]]:
    """从爬取结果中过滤匹配的机型图片
